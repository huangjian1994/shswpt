...
var nNo = 36;
var urlzw5 = document.location.hostname;
...
{
    var obj = document.getElementById("DPSDK_OCX");
    if(urlzw5 == '10.107.67.108')
      var szIp = '10.107.67.107';
//    var szIp = document.getElementById("textIP").value;
    else
      var szIp = '31.16.10.79'
...
//播放视频方法
function ButtonStartRealplayByWndNo_onclick(videoid,videoinfo) 
{
    var obj = document.getElementById("DPSDK_OCX");
    var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
    myCodes[nWndNo] = videoid;
//    ShowCallRetInfo(obj.DPSDK_StopRealplayByWndNo(gWndId, nWndNo), "停止播放视频");
	if(urlzw5 == '10.107.67.108')
      var nRet = obj.DPSDK_StartRealplayByWndNo(gWndId, nWndNo, videoid, 1, 1, 1);
//    var szIp = document.getElementById("textIP").value;
    else
      var nRet = obj.DPSDK_AsyncStartRealplayByWndNo(gWndId, nWndNo, videoid, 1, 1, 1);
	setTimeout(function() {
			ButtonSetOsdTxtByWndNo_onclick(nWndNo,videoinfo);
	}, 1000);
    ShowCallRetInfo(nRet, "播放视频");
}

//关闭全部视频方法
function ButtonStopRealplayByWndNo_all() 
{
    var obj = document.getElementById("DPSDK_OCX");
    
	var nWndNo = nNo;
	for (var int = 0; int <= nWndNo; int ++) {
		ShowCallRetInfo(obj.DPSDK_StopRealplayByWndNo(gWndId, int), "停止播放视频");
	}
...
function ButtonPtzDirection_onclick(nDirects)
{
	var obj = document.getElementById("DPSDK_OCX");
	var nWndNo = obj.DPSDK_GetSelWnd(gWndId);
    var szCameraId = myCodes[nWndNo];
    nDirect = nDirects;
    var nStep = document.getElementById("selectPtzDirectionStep").value;
    ShowCallRetInfo(obj.DPSDK_PtzDirection(szCameraId, nDirect, 3, 0), "方向控制");
    setTimeout(function() {
    	ShowCallRetInfo(obj.DPSDK_PtzDirection(szCameraId, nDirect, 3, 1), "方向控制");
	}, 1000);
}
...
function ButtonPtzCameraOperation_onclick(nOpers)
{
    ...
    setTimeout(function() {
    	ShowCallRetInfo(obj.DPSDK_PtzCameraOperation(szCameraId, nOper, nStep, 1), "镜头控制");
	}, 1000);
}
...
function ButtonSetOsdTxtByWndNo_onclick(nWndNonew,videoinfo)
{	
	
	var obj = document.getElementById("DPSDK_OCX");
	var nWndNo = nWndNonew;
    var text = '来源  '+videoinfo;
	//ShowCallRetInfo(obj.DPSDK_SetOsdTxtByWndNo( gWndId, nWndNo, text ), "设置Osd信息");
	ShowCallRetInfo(obj.DPSDK_SetOsdTxtExByWndNo( gWndId, nWndNo, text, 255,"黑体", 0,0,7,7), "设置Osd信息");
}
